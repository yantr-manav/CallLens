...
export default async function ReportPage({
  ...
  const detail =
    conversation.status === 'done'
      ? await store.getAnalysisDetail(conversation.id)
      : null;
    ...
    <AppShell user={user}>
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="flex min-w-0 items-center gap-2 text-xl font-semibold tracking-tight">
              ...
              <span className="truncate">{conversation.file_name}</span>
            </h1>
      ...
      {conversation.status === 'failed' && (
        <Card>
          <CardContent className="py-16 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Something went wrong while processing this transcript. Please try
              again.
            </p>
            <Button asChild variant="outline" size="sm" className="mt-4">
              <Link href="/analyze">
                Upload again <ArrowRight className="h-3.5 w-3.5" />
              </Link>
            </Button>
          </CardContent>
        </Card>
      ...
      {analysis && (
        <div className="space-y-6">
          ...
          <SentenceTable result={analysis.raw_json} />
        </div>
      )}
    </AppShell>
  );
