...
if (!url || !secret) {
  console.error('N8N_WEBHOOK_URL and N8N_WEBHOOK_SECRET must be set (.env or environment).');
  process.exit(2);
}
...
console.log(`  file=${file} turns=${turns.length} signature=${badSignature ? 'DELIBERATELY WRONG' : 'valid'}`);
...
if (badSignature) {
  const rejected =
    res.status === 401 || (json && typeof json.error === 'string' && json.ok !== true);
...
const checks = [
  ...
  ['overall_sentiment', ['positive', 'neutral', 'negative'].includes(r.overall_sentiment?.label)],
  ...
  ['sentence per turn', Array.isArray(r.sentences) && r.sentences.length === turns.length],
  ['evidence populated', Array.isArray(r.sentences) && r.sentences.some((s) => s.evidence)],
  ['reasoning.drivers', Array.isArray(r.reasoning?.drivers) && r.reasoning.drivers.length > 0],
